...
public class InterceptorChain extends EmptyInterceptor {
    ...
    public static InterceptorChain of(Class<? extends ChainableInterceptor>... interceptors) {
        return of(new ParameterContainer(), interceptors);
    }

    @SafeVarargs
    public static InterceptorChain of(ParameterContainer parameterContainer, Class<? extends ChainableInterceptor>... interceptors) {
        ...
        return new InterceptorChain(instantiate(parameterContainer, chain.next(), chain));
    }

    @SuppressWarnings("unchecked")
    private static ChainableInterceptor instantiate(ParameterContainer parameterContainer,
                                                    Class<? extends ChainableInterceptor> interceptorClass,
        ...
        for (int i = 0; i < parameterCount; i++) {
            ...
            if (parameterType.equals(Interceptor.class)) {
                if (next.hasNext()) {
                    parameters[i] = instantiate(parameterContainer, next.next(), next);
                } else {
                    parameters[i] = EmptyInterceptor.INSTANCE;
                }
            } else {
                parameters[i] = parameterContainer.get(parameterType);
            }
        }
